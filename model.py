...
def mrbeam(state, decoder, k=3, max_len=100, seq_len=400, sp_model=None, input=None, milestones=None):
	...
	for i in range(min(seq_len, max_len)):
		...
		state_ = [np.repeat(state[0],rep,axis=0).reshape((rep,state[0].shape[1])),
				  np.repeat(state[1],rep,axis=0).reshape((rep,state[1].shape[1]))]
		pred, _, _ = decoder.predict([live_sequences] + state_)

		#tops = [list(reversed(np.argsort(pred[n][i])[-k:])) for n in range(pred.shape[0])]
		...
		for n, prob_dist in enumerate(probs):
			...
			for m, token_prob in enumerate(prob_dist):
				...
				scores = live_scores[n] - np.log(token_prob)#np.log(token_prob/max_prob)
				if tops[n][m] == EOS:
		...
		try:
			live_scores, live_samples = zip(*heapq.nsmallest(k, zip(spare_scores, spare_samples), key=len_norm))
		except:
			import pdb; pdb.set_trace()
		if len(live_samples) == 0:
			break
	print()
	return sorted(zip(dead_scores + list(live_scores), dead_samples + list(live_samples)), key=len_norm, reverse=False)
...
class GenerationCallback(Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		try:
			...
			#print("Gold:", self.sp_model.DecodeIds([int(x) for x in output[0,] if x > 0]))
			encoder_in, decoder_in = input
		except ValueError as err:
			print(err)
			return

		for enc_in in [self.val_input[0][0:1,:], encoder_in][1:]:
			state = self.encoder_model.predict(enc_in[0:1,:])
			"""print("Input:", self.sp_model.DecodeIds([int(x) for x in enc_in[0,] if x > 0]))
			beams = mrbeam(state, self.decoder_model, k=30, max_len=epoch//8+30, seq_len=self.seq_len, sp_model=self.sp_model, milestones=enc_in[0,:])
			for n, (score, sample) in enumerate(beams):
				#if n % (len(beams)//8) == 0 or n >= len(beams)-5:
				if n >= len(beams)-5:
					print("%d --> %s (%.3f)" % (n, self.sp_model.DecodeIds([int(x) for x in sample]), len_norm((score, sample))))"""
...
class GenerationModel(object):
	...
	def __init__(self, vocab_size, input_len, sequence_len, args):
		...
		## Encoder model
		encoder_input = Input(shape=(input_len,))
		encoder_emb = shared_embeddings(encoder_input)
		#encoder_emb = TimestepDropout(0.1)(encoder_emb)
		...
		decoder_emb = TimestepDropout(0.3)(decoder_emb)
